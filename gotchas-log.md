...
- **Prerequisite for the owed loading run — do not skip it, or `neg-16` will be mis-scored the way `neg-08`/`neg-15` were.** The reachable owner is `superpowers:dispatching-parallel-agents`, a **plugin-namespaced** name, and this CHANGELOG already records that the runner's `_KEBAB_RE` (`run_evals.py:64`, `^[a-z0-9]+(?:-[a-z0-9]+)*$`) admits no colon — so a correct reply falls through to `NO_VERDICT`, the exact defect that produced 3 spurious non-verdicts across runs #4 and #5. The fixture is right; the instrument cannot express its answer. Fix or explicitly account for `_KEBAB_RE` **before** reading any score on this succession. The runner lives outside this skill, so this repo cannot close it.
