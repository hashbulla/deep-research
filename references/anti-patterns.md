...
Querying the solution space with the words of the presumed chain finds only that chain. "YouTube Watch Later export", "Instagram saved posts scraper" retrieve the tools that already named themselves after the problem — and miss every vendor that named itself after the *capability*. Sweep in **capability-class vocabulary**: unified messaging API, social-inbox aggregator, session-delegation provider, managed public-data API, MCP server for <platform>. The problem's own words are one query among many, never the whole sweep (AI-355). On GitHub the capability-class sweep is the **`topic:` facet** — an author-assigned, language-independent taxonomy immune to "my query words encode my hypothesis": ≥3 distinct combinations, star-sorted, before any keyword query ([github-research.md](github-research.md) §2, enforced by Rule 7b).
