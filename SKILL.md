...
5. **Classify the solution-space geometry.** Read `references/solution-space.md` — on EVERY run, this one reference is not on-demand — and decide: does the question admit a solution space ("what tool or approach solves this?"), and which platforms does it touch? Then plan the six-category sweep in `research-plan.md` under its "Solution-space sweep plan" section (`references/research-plan-template.md`), with the **own-stack sweep ordered first, before any web call**. A `not-applicable` geometry is declared, never skipped: the manifest is emitted on every run.
6. Decompose using the CoT pattern documented in `references/methodology.md` §5.1 and §8.2:
...
8. Write `research-plan.md` using the template in `references/research-plan-template.md`. The plan must include: classification, tier profile, sub-question list with proposed Tavily calls, domain allowlist preview, estimated total Tavily calls (respect 20 req/min research-endpoint rate limit — pace accordingly), expected contradiction axes, the solution-space sweep plan, stop conditions from `references/quality-gate.md`.
9. **Proceed to Phase 1 — no approval halt.** `research-plan.md` is the planning artifact (artifact #1 of the five-artifact contract), written before any retrieval call; step 3 already resolved any ambiguity or safety trigger. The one hard rule (`references/anti-patterns.md` A1): never fire `mcp__tavily__*` before the plan is written and the step-3 refinement, if triggered, has resolved.
...
Runs whenever Phase 0 declared the geometry applicable. Category taxonomy, status vocabulary, vendor vocabularies and the named registry list live in `references/solution-space.md` — this section is the order of operations only.

1. **Own stack first, before any web call.** `python3 <skill-dir>/scripts/stack_inventory.py` (local Bash, zero network) greps the operator's configured stack paths for tools already paid for or already integrated; the grep terms are recorded in the category's `queries` like any other query, and the capability terms it surfaces seed the vocabulary of the five web categories. Always full breadth, never waived for cost. No `~/.claude/deep-research/stack-paths.json` → `own-stack` is recorded `degraded` with the reason, never silently `empty`.
2. **Named registries next.** Sweep the MCP and package registries named in `references/solution-space.md`; record which ones were swept under the category's `registries` field.
3. **Then the web sweep, in capability-class vendor vocabulary — never the problem's own words** (`references/anti-patterns.md` B14). Tavily calls scale with `--length`: 4–6 (short) / 6–12 (standard) / 12–20 (exhaustive), spent **across** the five web categories, never concentrated in one.
4. **`empty` requires a control.** A category is recorded `empty` only alongside a control query whose `expected_hit` is a known-present item and whose `found` is true; without that proof the instrument is unverified and the status is `degraded` (B17).
5. A multi-platform question produces **one** manifest, with each finding tagged by `platform`. Buffer every category result (status, reason, date, queries, registries, findings) for `research-solution-space.json` — nothing is written to disk before Phase 6.
...
7. Draft `research-sources.json` and `research-evidence.json` rows in parallel. Schemas in `references/report-structure.md`. **No artifact file is written in this phase** — all five artifacts are written atomically at the end of Phase 6 (anti-pattern B11).
...
1. Spawn a decorrelated subagent on a different Claude model than the session (same mechanism as the Phase-5 entailment judge — Agent tool `model` override, `references/model-tiers.md`). Its sole mandate: name the categories or modalities the sweep did not cover, countersign every `waived` status, and challenge a `not-applicable` geometry declaration.
2. Inputs: the research question, the buffered solution-space manifest, and the category taxonomy from `references/solution-space.md` — **never the report prose**. The critic audits the sweep, not the findings. On `--confidential`, the input surface narrows per `references/solution-space.md` §"The completeness critic" (own-stack findings withheld — they never touched the web).
3. Resolve every finding as `swept` (re-run that category's sweep — **max 1 re-sweep iteration per run**), `waived` (a real reason, recorded verbatim), or `rejected` (with the note that rejects it). Record the whole pass in the manifest's `critic` block: `ran`, `model`, `waivers_reviewed`, `findings`.
...
3. **Render the solution space into the report** (`references/report-structure.md`): a "Solution-space benchmark" section whose table carries solution × class × cost × risk class × use-case coverage × verdict, every row citing a dated source; and, at the TOP of the report, every `declared_incompleteness` obligation quoted verbatim, plus the geometry declaration when `solution_space_applicable` is false.
4. Write all five artifacts atomically to the invocation CWD — this is the first and only artifact write of the run:
   ...
   - `research-solution-space.json` (solution-space manifest; schema at `tests/schema/research-solution-space.schema.json`)
5. **Deterministic gate verification (mandatory).** Run BOTH via Bash: `python3 <skill-dir>/scripts/verify_gates.py check-artifacts --sources research-sources.json --evidence research-evidence.json --length <length> --min-corroboration <n> [--since <date>]` and `python3 <skill-dir>/scripts/verify_gates.py check-solution-space --manifest research-solution-space.json`. On any violation, fix the artifacts (or move the offending claims to "Needs Verification") and re-run until both verdicts are PASS. Quote **both** JSON verdicts in the final chat message — self-reported metrics are not acceptable evidence. On a persistent FAIL after the bounded iterations (`references/quality-gate.md` §"Solution-space gates"), still deliver everything: the FAIL verdict becomes the first line of `research-report.md` — and of the Artifact when one is rendered (`--confidential` skips it) — and the report may not claim SOTA.
6. **Conditional delegation (only when `--suggest-tooling` is set; default OFF).** After all five artifacts are written and the gate verdicts are quoted, invoke the `suggest-tooling` sibling skill on the invocation CWD, passing the work-relevant topics flagged in Phase 0. This engine still emits exactly the five artifacts — `suggest-tooling` is a separate skill that writes `research-toolbox.md`. If the sibling skill is unavailable, note it in one line and finish; the five artifacts are unaffected. When the flag is unset (default), this step is skipped and the run is byte-identical to a normal run.
7. **Render the Artifact (final step).** Load the `artifact-design` skill, then publish ONE self-contained private page with the Artifact tool: the report, the solution-space benchmark table, the obligations header, the count of waived categories, and both gate verdicts. Skipped on `--confidential` — publishing is egress; record the skip in the Methodology note (the operator may lift it explicitly for a given run). If the Artifact tool is absent (headless surface), record the degradation in the Methodology note and finish — a missing Artifact never fails the run.
...
- **`research-solution-space.json`** — one manifest object per run. Schema at `tests/schema/research-solution-space.schema.json`; doctrine at `references/solution-space.md`. Required fields: `schema_version`, `generated`, `question`, `question_geometry`, `categories` (the closed set of 6, each with status + reason + date), `declared_incompleteness`, `critic`.
- **Artifact page** — the run's results rendered as a private page via the Artifact tool (Phase 6 final step). Not a file artifact: nothing is written to the CWD for it. Excluded on `--confidential` runs and when the Artifact tool is absent; either exclusion is recorded in the Methodology note.
...
- Do NOT output unrelated commentary, suggestions for further research beyond the plan, or meta-discussion of the skill's own design. Emit only the five artifacts.
- Do NOT consume a judge-refused universal ("no X exists", "X is the only option") or a source that declares its own inventory non-exhaustive as if it were fact. Either becomes a `declared_incompleteness` entry in the manifest with a stated obligation, quoted at the TOP of `research-report.md` (`references/anti-patterns.md` B16).
- Do NOT skip `research-solution-space.json` on any run. The manifest is mandatory including on `not-applicable` geometries, where it records the declaration and its reason rather than an empty sweep.
...
- **`stack-paths.json` config absent.** `scripts/stack_inventory.py` finds no `~/.claude/deep-research/stack-paths.json` (template: `stack-paths.json.example`). Record `own-stack` as `degraded` with that reason and continue the other five categories — a missing own-stack inventory is visible, never a silent `empty`.
- **Artifact tool unavailable (headless surface).** `allowed-tools` pre-approves the Artifact tool, it does not guarantee its presence. If the tool is absent, record the degradation in the Methodology note and finish the run on the five file artifacts. Never fail, retry, or substitute another publishing path.
...
- **`references/solution-space.md`** — solution-space doctrine: the closed set of 6 categories, the status and capability-class vocabularies, the completeness-critic contract, and the rules `verify_gates.py check-solution-space` enforces. **Read at Phase 0 on EVERY run** — the one exception to the on-demand rule above.
