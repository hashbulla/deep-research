...
- **[DIM-1-01] [WARNING]** `loading_matrix.json:4-6` — matrix covers **opus only**, on the branch whose sharpest change is the routing prompt. A weakened description degrades on weaker routers first, and the repo's own history puts the failure exactly there: run #4's `neg-08` leak landed on **sonnet** (CHANGELOG.md:81; gotchas-log.md:39 records the same cell leaking on sonnet *and* haiku under ablation). Fix: run `run_evals.py --suite loading --models opus,sonnet,haiku --backend cli` before any release tag; require >=14/15 positives and >=15/16 negatives per model (`evals/rubric.md:17`). Do not publish "routing verified" — publish "verified on opus".
  Citations:
  - Skills for enterprise — Claude Platform Docs. https://platform.claude.com/docs/en/agents-and-tools/agent-skills/enterprise. (Tier 1, Admiralty A1) — "Require testing across the models your organization uses (Haiku, Sonnet, Opus), because Skill effectiveness varies by model."
  - Skill authoring best practices — Claude Platform Docs. https://platform.claude.com/docs/en/agents-and-tools/agent-skills/best-practices. (Tier 1, Admiralty A1) — "Test with all models you plan to use. Skills act as additions to models, so effectiveness depends on the underlying model. [...] What works perfectly for Opus might need more detail for Haiku."
  - Demystifying evals for AI agents — Anthropic Engineering. https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents. (Tier 1, Admiralty A2) — "Because model outputs vary between runs, we run multiple trials to produce more consistent results." (grounds the n=1 half of this finding and DIM-1-02.)
...
- **[DIM-2-01] [WARNING]** lint check#13, SKILL.md:7 — **no `## Examples` section.** Deliberate (CHANGELOG.md:43, gotchas-log.md:21: "`Examples` folded into `References` with its content intact") and the content is verifiably intact and reachable (`references/examples.md`, 1,481 tok, pointed to at SKILL.md:150 with its read-moment). Not content loss — a body-contract deviation from the generator's required-section list. Fix: add `## Examples` before `## References` carrying only the pointer sentence currently embedded at SKILL.md:150, and delete that clause from the References bullet. Net ~+2 lines; the 150-line gate has zero headroom, so pair with DIM-2-02.
  Citations:
  - Agent Skills overview — Claude Platform Docs. https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview. (Tier 1, Admiralty A1) — the canonical SKILL.md template published under "Skill structure" carries exactly two body headings, `## Instructions` and `## Examples`.
  - The Complete Guide to Building Skills for Claude — Anthropic. https://resources.anthropic.com/hubfs/The-Complete-Guide-to-Building-Skill-for-Claude.pdf. (Tier 1, Admiralty A2) — the "During development" authoring checklist lists "Examples provided" as a pre-upload item.

  Contradicting evidence:
  - Specification — Agent Skills. https://agentskills.io/specification. (Tier 1, Admiralty A1) — "The Markdown body after the frontmatter contains the skill instructions. **There are no format restrictions.** Write whatever helps agents perform the task effectively. Recommended sections: Step-by-step instructions / Examples of inputs and outputs / Common edge cases." The frontmatter table marks only `name` and `description` as Required; **no body section is required by the published spec** — `Examples` is *recommended*.
  - Agent Skills overview — Claude Platform Docs. https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview. (Tier 1, Admiralty A1) — the same page that publishes the two-heading template states "Required fields: `name` and `description`" and imposes no required body heading.
  - Grounder's note, per constitution: this contradicting evidence is **surfaced, not applied**. Severity remains **WARNING**. The finding's asserted authority is the *generator's* required-section contract (lint check#13), not the public spec, and the finding already characterises the deviation as a body-contract matter rather than content loss. The user adjudicates whether a generator-local contract that the published spec does not mandate should carry WARNING weight.
...
- **[DIM-3-01] [WARNING]** CHANGELOG.md:21 and :27 — the documented justification quotes **"load 7,199 tok … 301 tok of headroom"**. This branch invalidated both: HEAD measures **7,240 tok / 260 tok headroom**. The *substance* survives; the *figures* are stale — and the entire point of that entry was measurement discipline after the run-#3 D3 failure (gotchas-log.md:67-70, "no re-measurement of a body that grew +49%"). CHANGELOG.md:37's "absorbed the new Phase-6 step at **net-zero lines**, so the 150-line budget gate still passes" is **true exactly as written**; the defect is that no token figure was re-measured beside it. Fix: append one line to the `[Unreleased]` accounting entry — `Load tier re-measured after the Phase-6 step: 7,240 tok (was 7,199 pre-branch; 7,099 post-compression) — 260 tok under the 7,500 local hard cap.`
  Citations:
  - Specification — Agent Skills. https://agentskills.io/specification. (Tier 1, Admiralty A1) — "Instructions (< 5000 tokens recommended): The full `SKILL.md` body is loaded when the skill is activated." The budget the stale 7,199 figure is measured against is denominated in tokens, so a token figure is the load-bearing number in that justification.
  - Designing, Refining, and Maintaining Agent Skills at Perplexity — Perplexity Research. https://research.perplexity.ai/articles/designing-refining-and-maintaining-agent-skills-at-perplexity. (Tier 2, Admiralty B2) — "Ideally, the body text does not exceed 5,000 tokens. Even then, you want every sentence to matter because once you load a Skill, the rest of the conversation has to pay that until you hit the compaction boundary."
  - Evaluating skill output quality — Agent Skills. https://agentskills.io/skill-creation/evaluating-skills. (Tier 1, Admiralty A2) — the published iteration loop requires re-running and re-grading after each applied change ("Rerun all test cases in a new `iteration-/` directory. Grade and aggregate the new results."), i.e. figures carried forward from a prior iteration are not evidence for the current one.
- **[DIM-3-02] [WARNING]** `tests/check-skill-length.sh:16` (`LIMIT="${SKILL_LINE_LIMIT:-150}"`, measured with `wc -l`), wired at `.github/workflows/validate.yml:30-31` — **the branch's new budget gate counts lines, and no gate anywhere counts tokens.** Verified: `grep -rln "token_budget\|LOAD_HARD\|7500" tests/ .github/` returns **nothing**. The script's header states the rationale in token terms ("SKILL.md is the prompt served on every activation, so its length is a budget"), but newlines are not what the model pays. **This branch is its own counter-example: -59 lines and +41 tokens simultaneously.** A future densification can pass the 150-line gate while pushing the load tier through 7,500 unobserved. Fix: add a CI step after the length check — `python3 <skill-generator>/scripts/token_budget.py . --json`, failing on exit 1 and printing tier warnings otherwise; or a stdlib character-count proxy calibrated against the current 7,240-tok/~28,000-char ratio to stay dependency-free. Keep the line gate: it guards attention, which a token gate does not.
  Citations:
  - Specification — Agent Skills. https://agentskills.io/specification. (Tier 1, Admiralty A1) — the progressive-disclosure tier table denominates the activated-body budget in tokens: "Instructions (< 5000 tokens recommended): The full `SKILL.md` body is loaded when the skill is activated."
  - Agent Skills overview — Claude Platform Docs. https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview. (Tier 1, Admiralty A1) — the loading table states the same in tokens: "Level 2: Instructions | When Skill is triggered | Under 5k tokens | SKILL.md body with instructions and guidance."
  - Designing, Refining, and Maintaining Agent Skills at Perplexity — Perplexity Research. https://research.perplexity.ai/articles/designing-refining-and-maintaining-agent-skills-at-perplexity. (Tier 2, Admiralty B2) — "In the index, every token is important. The loaded Skill body is more relaxed" — the unit of account for what the model pays is the token, not the newline.

  Contradicting evidence:
  - Skill authoring best practices — Claude Platform Docs. https://platform.claude.com/docs/en/agents-and-tools/agent-skills/best-practices. (Tier 1, Admiralty A1) — the section titled **"Token budgets"** states its operative rule in **lines**: "Keep SKILL.md body under 500 lines for optimal performance. If your content exceeds this, split it into separate files using the progressive disclosure patterns described earlier." A line-count gate therefore has direct vendor backing as a published budget check.
  - Best practices for skill creators — Agent Skills. https://agentskills.io/skill-creation/best-practices. (Tier 1, Admiralty A1) — "The specification recommends keeping `SKILL.md` under 500 lines **and** 5,000 tokens" — the two denominations are published as co-equal caps, not as proxy-and-referent.
  - Grounder's note, per constitution: surfaced, not applied. Severity remains **WARNING**, and this evidence does not rebut the finding's actual claim. The published guidance names **two** caps; the branch gates **one** of them (`wc -l` at 150) and `grep` proves no gate counts tokens, so the un-gated published cap is un-gated regardless of which denomination a given page leads with. The branch's own measurement (-59 lines, +41 tokens) demonstrates the two caps can move in opposite directions.
...
- **[DIM-4-01] [WARNING]** `evals/sycophancy-probes.jsonl` (5 rows) and `evals/benchmark-testset.jsonl` (5 rows) are executed by **nothing** — `run_evals.py --suite` accepts only `loading|progressive|e2e` (CHANGELOG.md:57). This is a *presence*-level defect inside D4's own input surface, not an executability caveat: two files sit in `evals/` that no suite value can name, and `benchmark-testset.jsonl` is on a **declared 4-weekly re-validation cadence** (gotchas-log.md:136) no runner reads, so its staleness can never surface mechanically. Fix: take the repo's own proposal — relocate both to `evals/manual/` (or `docs/eval-protocols/`) so non-executability is visible from the path, and add one line to `evals/rubric.md` naming who runs them and when. Deferring the `--suite` extension is fine; leaving them indistinguishable from executed fixtures is not.
  Citations:
  - Evaluating skill output quality — Agent Skills. https://agentskills.io/skill-creation/evaluating-skills. (Tier 1, Admiralty A1) — the published protocol binds fixtures to execution and grading: "Add assertions to each test case in `evals/evals.json`" and "Rerun all test cases [...] Grade and aggregate the new results"; for code-checkable assertions "use a verification script — scripts are more reliable than LLM judgment for mechanical checks and reusable across iterations."
  - Demystifying evals for AI agents — Anthropic Engineering. https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents. (Tier 1, Admiralty A2) — the published vocabulary makes a grader constitutive of a task: "A grader is logic that scores some aspect of the agent's performance. A task can have multiple graders, each containing multiple assertions." A fixture file no runner names has no grader and therefore emits no signal.
  - Skills for enterprise — Claude Platform Docs. https://platform.claude.com/docs/en/agents-and-tools/agent-skills/enterprise. (Tier 1, Admiralty A1) — the lifecycle makes eval results the trigger for action ("Evaluation results signal when to act"), which a declared 4-weekly re-validation cadence no runner reads cannot supply.
...
- **[DIM-6-01] [WARNING]** SKILL.md:3 — post-edit, the negative-boundary list names **no skill peer at all**: two tools and one slash command. Meanwhile the peer the router *empirically chooses* on the skill's own release-blocker boundary — `superpowers:dispatching-parallel-agents`, per CHANGELOG.md:41 and the runs-#4/#5 record — appears only in a fixture `boundary` and at `evals/rubric.md:7`: in the test contract, **not on the routing surface**. Tier 10 asks for explicit negative boundaries to nearest peers; the nearest *measured* peer is unnamed. The run-#5 adjudication deliberately deferred this edit pending measurement and I am not overriding it. Fix: do **not** edit on the current n=1 evidence — run the n>=3 measurement DIM-1-02 already owes, and if `neg-16` leaks on any model at >=1-in-3, add the peer by name to the `Do NOT load` clause (346 chars of headroom under the cap).
  Citations:
  - The Complete Guide to Building Skills for Claude — Anthropic. https://resources.anthropic.com/hubfs/The-Complete-Guide-to-Building-Skill-for-Claude.pdf. (Tier 1, Admiralty A2) — the published remedy for over-triggering names the competing **skill** on the description surface: "Add negative triggers — `description: Advanced data analysis for CSV files. Use for statistical modeling, regression, clustering. Do NOT use for simple data exploration (use data-viz skill instead).`"
  - Skills for enterprise — Claude Platform Docs. https://platform.claude.com/docs/en/agents-and-tools/agent-skills/enterprise. (Tier 1, Admiralty A1) — coexistence is a named eval dimension: "Does adding this Skill degrade other Skills? [...] New Skill's description is too broad, stealing triggers from existing Skills", with the remedy "Consolidate overlapping Skills or narrow descriptions."
  - Optimizing skill descriptions — Agent Skills. https://agentskills.io/skill-creation/optimizing-descriptions. (Tier 1, Admiralty A1) — "The most valuable negative test cases are near-misses — queries that share keywords or concepts with your skill but actually need something different. These test whether the description is precise, not just broad." Grounds the finding's point that the nearest *measured* peer is the one that must be named, and (consistent with the finding's own deferral) that the case is settled by measurement.
